...
def call_gemini_native(base_url: str, model: str, prompt: str, api_key: str, proxy_url: str) -> str:
...
def call_openai_compatible(base_url: str, model: str, prompt: str, env_tag: str, api_key: str) -> str:
...
def build_caller(provider: str):
    ...
    if provider == "gemini":
        ...
        return (lambda prompt: call_gemini_native(base, model, prompt, api_key, proxy_url)), None
    if provider == "gateway":
        ...
        return (lambda prompt: call_openai_compatible(base, model, prompt, env_tag, api_key)), None
    if provider == "local":
...
def main() -> None:
    ...
    if not input_jsonl or not output_jsonl:
        fail("缺少必要参数: input_jsonl, output_jsonl。")
    ...
    try:
        max_workers = int(params.get("max_workers") or 16)
    except (TypeError, ValueError):
