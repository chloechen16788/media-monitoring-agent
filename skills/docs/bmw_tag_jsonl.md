...
【调用方式】: 必须严格以 JSON 字符串形式传入参数，禁止使用单引号。示例: python skills/executor/bmw_tag_jsonl.py "{\"input_jsonl\":\"./workspace/bmw_demo/raw/batch_000.jsonl\",\"output_jsonl\":\"./workspace/bmw_demo/annotated/batch_000.jsonl\",\"provider\":\"gemini\"}"
【参数说明】 (以 JSON 字符串形式传入):
  - input_jsonl (必需): 待标注的 JSONL 批次路径（由 es_export_jsonl_batches 生成）。
  - output_jsonl (必需): 标注结果输出路径。
  ...
  - max_workers: 并发数，默认 16，上限 64。
  - max_records: 单批最大条数（成本围栏），默认 1500，超出报错要求分批。
  - progress_file: 可选。进度写入路径（旁路文件，不打 stdout），形如 {"done","total","ok","fail"}。
  - manifest_path: 可选。完成后回写该 manifest 中对应批次状态为 annotated。
  - batch_id: 可选。配合 manifest_path 指定回写的批次 id。
  - prompt_override: 可选。覆盖默认 BMW 判定 prompt 模板（建议使用 {title} + {text} + {text_label}；兼容旧占位符 {content}）。
【环境变量】(标注模型隔离):
...
【返回格式】: JSON。成功: {"ok": true, "data": {"input_jsonl","output_jsonl","provider","total","succeeded","failed","label_distribution"}}；失败: {"ok": false, "error", "hint"}。
...
python skills/executor/bmw_tag_jsonl.py "{\"input_jsonl\":\"./workspace/bmw_demo/raw/batch_000.jsonl\",\"output_jsonl\":\"./workspace/bmw_demo/annotated/batch_000.jsonl\",\"provider\":\"gateway\",\"max_workers\":4}"

【示例 - 显式切到标题+正文2000字】:
python skills/executor/bmw_tag_jsonl.py "{\"input_jsonl\":\"./workspace/bmw_demo/raw/batch_000.jsonl\",\"output_jsonl\":\"./workspace/bmw_demo/annotated/batch_000.jsonl\",\"provider\":\"gemini\",\"input_text_mode\":\"content\",\"content_max_chars\":2000}"
